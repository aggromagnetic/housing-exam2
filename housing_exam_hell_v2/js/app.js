...
(function () {
    ...
    // 🛡️ Master Purged Needs-Edit Keys (53 old baked items + 4 legacy items)
    ...
    const ExamEngine = {
        ...
        GENERIC_STOP_WORDS: new Set([
            ...
            "어느", "하나", "각호", "해당", "또는", "위한", "통해", "대해", "암기", "시기",
            // 🛡️ 법령/문항 공통 어휘 및 문장 발문
            "법령", "법령상", "조항", "규정상", "기준상", "해당하지", "필요한",
            "경우에", "경우에는", "이내에", "이내에는",
            "자에게", "자에게는", "자로서", "자로서의",
            // 🛡️ 부정 표현 (문제 발문 및 서술어 부정형 오매칭 방지)
            "않은", "않는", "않고", "않아", "않다", "않음", "않으며", "않거나", "않게", "않도록",
            "아닌", "아니", "아님", "아닐", "아니한", "아니하며", "아니하고", "아니하는", "아니하다",
            "못한", "못하는", "못함", "못하고",
            // 🛡️ 1글자 체언 + 조사/어미 결합형 배제
            "자는", "자가", "자의", "자로", "자에", "자를", "자도", "자만",
            "날에", "날의", "날을", "날이",
            "때는", "때에", "때의", "때만", "때도",
            "중에", "중의", "중을", "중은", "중이",
            "전에", "전의", "전을", "전은", "전이",
            ...
            "받아", "받고", "받는", "받음", "받으며", "받거나", "받았", "받지", "받게", "받을", "받도록",
            "하고", "하며", "하면", "하여", "하던", "한", "하여야", "하지", "하게", "하도록", "하기", "했다", "한다", "하되", "하려", "해야", "한단",
            "되고", "되며", "되면", "되어", "되는", "되거나", "된", "되어야", "되지", "되게", "되도록", "되기", "된다", "돼야", "되려", "되되",
            "있고", "있으며", "있어", "있는", "있거나", "있어야", "있다", "있을", "있게",
            "없고", "없으며", "없어", "없는", "없거나", "없어야", "없다", "없을", "없이", "없게",
            "위해", "위해서", "위하여", "위함",
            "대해", "대해서", "대하여",
            "따라", "따르고", "따르면",
            "통해", "통하여",
        ...
        SUFFIX_2CHAR_REGEX: /(으로|에서|에게|부터|까지|마다|따라|따른|관한|대한|위한|통해|대해|하기|하는|하여|하며|하면|되고|되며|되면|되어|되는|되기|받고|받는|받은|받아|지나|하지|하게|되게|받게|하려|하되)$/,
        ...
        cleanWord(w) {
            ...
            // Up to 2-pass suffix removal to handle compound endings (e.g., 건설업자에게는 -> 건설업자, 경우에는 -> 경우)
    ...
    const state = {
        subject: (function() {
            try {
                return sessionStorage.getItem('hell_subject') || '관계법규';
            } catch (e) {
                return '관계법규';
            }
        })(),
    ...
    function triggerHaptic(duration = 15) {
        try {
            ...
            if (window.WebAppInterface && typeof window.WebAppInterface.vibrateSoftly === 'function') {
                window.WebAppInterface.vibrateSoftly(Array.isArray(duration) ? duration[0] : duration);
            }
        } catch (e) {}
    }
    ...
    function showScreen(screenKey) {
        ...
        if (screenKey === 'manager') {
            ...
            if (elements.header.modeTitle) {
                const semver = window.APP_SEMVER || 'v.2.0.0-PRO';
                elements.header.modeTitle.innerHTML = `<i class="fa-solid fa-layer-group text-rose-500"></i> 오답 관리 & 전체 문제 에디터 <span class="version-tag">${semver}</span>`;
            }
        } else {
        ...
        if (screenKey === 'home') {
            ...
            if (elements.header.modeTitle) {
                const semver = window.APP_SEMVER || 'v.2.0.0-PRO';
                elements.header.modeTitle.innerHTML = `<i class="fa-solid fa-fire text-amber-500"></i> 주관사 2차 문제지옥 <span class="version-tag">${semver}</span>`;
            }
    ...
    function setSubject(subj) {
        ...
        triggerHaptic(15);
        try {
            sessionStorage.setItem('hell_subject', subj);
            localStorage.removeItem('hell_subject'); // 🛡️ Clean legacy stale storage so next cold start defaults to 관계법규 cleanly
        } catch (e) {}
        if (elements.body) {
    ...
    /**
     * ⚡ Instant Synchronous In-Memory Stat Update (0ms UI synchronization)
     * Guarantees that renderQuestion immediately reflects the updated wrongCount and score
     * without waiting for async IndexedDB disk I/O.
     */
    function applyOptimisticAnswerStat(qKey, isCorrect) {
        const curStat = state.statsMap[qKey] || { qKey, tryCount: 0, wrongCount: 0, totalWrongCount: 0, correctCount: 0, weight: 1.0 };
        const nextTryCount = (curStat.tryCount || 0) + 1;
        let nextWrongCount = curStat.wrongCount || 0;
        let nextTotalWrong = curStat.totalWrongCount || (curStat.wrongCount ? curStat.wrongCount : 0);
        let nextCorrectCount = curStat.correctCount || 0;
        let nextWeight = curStat.weight || 1.0;

        if (isCorrect) {
            nextCorrectCount++;
            if (nextWrongCount > 0) nextWrongCount = Math.max(0, nextWrongCount - 1);
            if (nextWrongCount === 0) nextWeight = 1.0;
            else if (nextWeight > 1.4) nextWeight = 1.4;
            else if (nextWeight > 1.2) nextWeight = 1.2;
            else nextWeight = 1.0;
        } else {
            nextWrongCount++;
            nextTotalWrong++;
            if (nextWrongCount === 1) nextWeight = 1.2;
            else if (nextWrongCount === 2) nextWeight = 1.4;
            else if (nextWrongCount === 3) nextWeight = 1.6;
            else nextWeight = 1.8;
        }
        ...
        const updated = {
            ...
            totalWrongCount: nextTotalWrong,
            correctCount: nextCorrectCount,
            weight: nextWeight,
            lastAttempt: new Date().toISOString(),
            lastResult: isCorrect
        };
        state.statsMap[qKey] = updated;
        return updated;
    }
    ...
    async function renderQuestion(index) {
        ...
        if (prevIdx !== undefined && prevIdx !== index && state.questions && state.questions[prevIdx]) {
            ...
            if (state.userAnswers[prevIdx] !== undefined && state.firstAttemptResults[prevIdx] === undefined) {
                ...
                applyOptimisticAnswerStat(prevQ.qKey, gradeRes.isCorrect);
        ...
        let wIcon = '🌱 기본';
        if (wrongCount > 0) {
            if (isLv4) wIcon = `🔥 지옥 (오답 ${wrongCount}회)`;
            else if (isLv3) wIcon = `🚨 취약 (오답 ${wrongCount}회)`;
            else if (isLv2) wIcon = `⚠️ 주의 (오답 ${wrongCount}회)`;
            else if (isLv1) wIcon = `⚡ 복습 (오답 ${wrongCount}회)`;
        } else if (stat.tryCount > 0) {
            wIcon = '🌱 기본 (완료)';
        }
    ...
    async function gradeCurrentQuestion() {
        ...
        if (state.firstAttemptResults[idx] === undefined) {
            ...
            // 🚀 Instant in-memory optimistic stat sync before rendering
            applyOptimisticAnswerStat(q.qKey, gradeRes.isCorrect);
    ...
    function nextQuestion() {
        ...
        if (state.mode === 'infinite' || state.mode === 'part') {
            ...
            if (curQ && state.userAnswers[curIdx] !== undefined && state.firstAttemptResults[curIdx] === undefined) {
                ...
                applyOptimisticAnswerStat(curQ.qKey, gradeRes.isCorrect);
        ...
        pagedList.forEach((q) => {
            ...
            if (btnQuickDel) {
                btnQuickDel.addEventListener('click', async (e) => {
                    ...
                    state.statsMap[q.qKey] = { weight: 1, wrongCount: 0, tryCount: 0, totalWrongCount: 0, resetAt: new Date().toISOString() };
    ...
    function initEventListeners() {
        ...
        if (elements.manager.btnDeleteWrong) {
            elements.manager.btnDeleteWrong.addEventListener('click', async () => {
                ...
                state.statsMap[qKey] = { weight: 1, wrongCount: 0, tryCount: 0, totalWrongCount: 0, resetAt: new Date().toISOString() };
        ...
        if (elements.quiz && elements.quiz.btnNext) {
            elements.quiz.btnNext.addEventListener('click', () => {
                triggerHaptic(15);
                nextQuestion();
            });
        }
        if (elements.quiz && elements.quiz.btnPrev) {
            elements.quiz.btnPrev.addEventListener('click', () => {
                triggerHaptic(15);
                prevQuestion();
            });
        }
        if (elements.quiz && elements.quiz.btnToggleExp) {
...
🤖 요청사항: 위 문제의 정답과 해설, 최신 개정 법령 반영 여부, 정확한 명칭, 수치 등을 반드시 웹검색을 통해 면밀히 검증하고 보완 사항을 알려주세요.`;
    ...
    window.addEventListener('DOMContentLoaded', async () => {
        initDOMElements();
        setSubject(state.subject); // ⚡ Instant 0ms subject sync to prevent visual jump/flicker
        await IDBStore.init();
        ...
        // 2. Sanitize local needs-edit (permanently strip all 53 previously baked questions)
        try {
            const rawNeeds = JSON.parse(localStorage.getItem('housing_exam_needs_edit') || '{}');
            ...
            if (needsChanged) {
                localStorage.setItem('housing_exam_needs_edit', JSON.stringify(rawNeeds));
            }
        } catch (e) {}
