...
(function () {
        ...
        async importBackupJSON(backupData) {
            ...
            if (db) {
                try {
                    ...
                    console.error('app.js IDBStore.importBackupJSON error:', e);
                }
            }
    ...
    function renderSubjectiveBlanks(q, index) {
        ...
        entries.forEach(([k]) => {
            ...
            input.addEventListener('focus', () => {
                if (window.AndroidBridge && typeof window.AndroidBridge.hideNavigationBars === 'function') {
                    window.AndroidBridge.hideNavigationBars();
                }
            });
    ...
    function renderManagerList(tabName = state.managerTab, filterSubj = state.managerFilter, query = state.managerSearchQuery) {
        ...
        pagedList.forEach((q) => {
            ...
            const btnQuickUnflag = card.querySelector('.mgr-card-quick-unflag');
            if (btnQuickUnflag) {
                btnQuickUnflag.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    ...
                    if (state.managerTab === 'needs_edit') {
                        ...
                        btnQuickUnflag.remove();
                        const flagBadge = card.querySelector('.mgr-status-flag');
                    ...
                    showToast(`🏳️ [${cleanChap} ${q.id}번] 수정 필요 목록에서 완전히 제외되었습니다.`);
                });
            }
        ...
        if (btnClearAllFlags) {
            btnClearAllFlags.addEventListener('click', async () => {
                ...
                if (confirm(`수정 필요 목록에 등록된 ${count}개 항목을 모두 초기화(비우기)하시겠습니까?`)) {
                    await IDBStore.clearAllNeedsEdit();
                    state.needsEditMap = {};
                    renderManagerList();
                    if (elements.manager.btnFlagToggle) elements.manager.btnFlagToggle.classList.remove('active');
                    if (elements.manager.flagText) elements.manager.flagText.textContent = '수정필요';
                    if (elements.manager.flagBadge) elements.manager.flagBadge.style.display = 'none';
                    if (elements.manager.cntNeedsEdit) elements.manager.cntNeedsEdit.textContent = '0';
...
🤖 요청사항: 위 문제의 정답과 해설, 최신 개정 법령 반영 여부, 정확한 명칭, 수치 등을 반드시 웹검색을 통해 면밀히 검증하고 보완 사항을 알려주세요.`;
        ...
        if (elements.header.btnCloudSync) {
            elements.header.btnCloudSync.addEventListener('click', async () => {
                ...
                if (ok) {
                    ...
                    const wrongCount = Object.values(state.statsMap || {}).filter(s => (s.wrongCount || 0) > 0).length;
                    const totalAttempted = Object.values(state.statsMap || {}).filter(s => (s.tryCount || 0) > 0).length;
                    showToast(`☁️ 클라우드 동기화 완료! (학습 기록: ${totalAttempted}개 / 오답: ${wrongCount}개 / 수정: ${Object.keys(state.customEdits || {}).length}개)`);
                } else {
    ...
    // 🛡️ 53 Recovered Needs-Edit Questions Master Safety Net
    const RECOVERED_NEEDS_EDIT_53 = {"관계법규_choice_CHAPTER 02 공동주택관리법_88": {"qKey": "관계법규_choice_CHAPTER 02 공동주택관리법_88", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 02 공동주택관리법", "question": "공동주택관리법령에 관한 내용으로 옳지 않은 것은?"}, "관계법규_short_CHAPTER 09 소방기본법_06": {"qKey": "관계법규_short_CHAPTER 09 소방기본법_06", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 09 소방기본법", "question": "｢소방기본법령｣상 화재 등의 통지 및 위반 시 과태료에 관한 설명이다. ( ㉠ ), ( ㉡ )에 들어갈 알맞은 용어와 아라비아 숫자를 쓰시오."}, "관계법규_choice_CHAPTER 11 소방시설 설치 및 관리에 관한 법률_15": {"qKey": "관계법규_choice_CHAPTER 11 소방시설 설치 및 관리에 관한 법률_15", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 11 소방시설 설치 및 관리에 관한 법률", "question": "소방시설 설치 및 관리에 관한 법령상 ‘우수품질 제품에 대한 인증’에 관한 설명으로 옳지 않은 것은?"}, "관계법규_short_CHAPTER 01 주택법_89": {"qKey": "관계법규_short_CHAPTER 01 주택법_89", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 01 주택법", "question": "다음은 ｢주택법령｣상 입주자저축 등에 관한 규정의 일부이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 알맞은 용어를 각각 쓰시오."}, "관계법규_choice_CHAPTER 04 공공주택 특별법_14": {"qKey": "관계법규_choice_CHAPTER 04 공공주택 특별법_14", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 04 공공주택 특별법", "question": "공공주택 특별법령상 공공주택지구(이하 ‘주택지구’라 한다)의 조성에 관한 설명으로 옳지 않은 것은?"}, "관리실무_short_CHAPTER 11 시설관리_109": {"qKey": "관리실무_short_CHAPTER 11 시설관리_109", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 11 시설관리", "question": "주택건설기준 등에 관한 규정상 공동주택의 복도에 관한 규정의 일부이다. ( )에 들어갈 아라비아 숫자를 쓰시오."}, "관계법규_short_CHAPTER 02 공동주택관리법_32": {"qKey": "관계법규_short_CHAPTER 02 공동주택관리법_32", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 02 공동주택관리법", "question": "｢공동주택관리법령｣상 입주자대표회의의 의결 및 주민공동시설 등에 관한 규정의 일부이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 알맞은 용어를 각각 쓰시오."}, "관리실무_choice_CHAPTER 04 관리조직 및 입주자대표회의_83": {"qKey": "관리실무_choice_CHAPTER 04 관리조직 및 입주자대표회의_83", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 04 관리조직 및 입주자대표회의", "question": "공동주택관리법령상 하자심사·분쟁조정위원회의 회의 등에 관한 설명으로 옳은 것은?"}, "관리실무_short_CHAPTER 07 입주자관리 및 자치규약_04": {"qKey": "관리실무_short_CHAPTER 07 입주자관리 및 자치규약_04", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 07 입주자관리 및 자치규약", "question": "공동주택관리법 제73조 중앙분쟁조정위원회의 구성 등에 관한 규정의 일부이다. ( )에 들어갈 아라비아 숫자를 쓰시오."}, "관리실무_choice_CHAPTER 04 관리조직 및 입주자대표회의_15": {"qKey": "관리실무_choice_CHAPTER 04 관리조직 및 입주자대표회의_15", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 04 관리조직 및 입주자대표회의", "question": "공동주택관리법령상 입주자대표회의에 관한 설명으로 옳은 것은?"}, "관계법규_short_CHAPTER 02 공동주택관리법_특강_윤동섭_SA_39": {"qKey": "관계법규_short_CHAPTER 02 공동주택관리법_특강_윤동섭_SA_39", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 02 공동주택관리법", "question": "공동주택관리법령상 ‘안전점검’에 관한 설명이다. ( )에 들어갈 용어 및 아라비아 숫자를 쓰시오."}, "관계법규_short_CHAPTER 08 시설물의 안전 및 유지관리에 관한 특별법_05": {"qKey": "관계법규_short_CHAPTER 08 시설물의 안전 및 유지관리에 관한 특별법_05", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 08 시설물의 안전 및 유지관리에 관한 특별법", "question": "｢시설물의 안전 및 유지관리에 관한 특별법｣에 관한 규정이다. ( ㉠ ), ( ㉡ )에 들어갈 알맞은 용어를 각각 쓰시오."}, "관리실무_choice_CHAPTER 08 사무 및 인사관리_199": {"qKey": "관리실무_choice_CHAPTER 08 사무 및 인사관리_199", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "국민건강보험법령상 이의신청 및 심판청구에 관한 설명으로 옳은 것을 모두 고른 것은?"}, "관리실무_short_CHAPTER 03 공동주택의 관리방법_57": {"qKey": "관리실무_short_CHAPTER 03 공동주택의 관리방법_57", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 03 공동주택의 관리방법", "question": "다음은 주택임대관리업의 현황 신고 및 보고에 관한 내용이다. 괄호 ( ㄷ )에 들어갈 알맞은 숫자를 쓰시오."}, "관계법규_short_CHAPTER 05 건축법_44": {"qKey": "관계법규_short_CHAPTER 05 건축법_44", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 05 건축법", "question": "｢건축법｣상 특별자치시장ㆍ특별자치도지사 또는 시장ㆍ군수ㆍ구청장은 건축허가, 건축신고, 사용승인 등 건축과 관련된 민원을 종합적으로 접수하여 처리할 수 있는 ( ㉠ )을(를) 설치ㆍ운영하여야 한다."}, "관계법규_short_CHAPTER 02 공동주택관리법_83": {"qKey": "관계법규_short_CHAPTER 02 공동주택관리법_83", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 02 공동주택관리법", "question": "｢공동주택관리법｣상 하자심사·분쟁조정위원회의 조정등의 처리기간 및 절차에 관한 규정이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 아라비아 숫자와 용어를 각각 쓰시오."}, "관리실무_choice_CHAPTER 08 사무 및 인사관리_138": {"qKey": "관리실무_choice_CHAPTER 08 사무 및 인사관리_138", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "산업재해보상보험법령상 심사청구 및 재심사청구에 관한 설명으로 옳지 않은 것을 모두 고른 것은?"}, "관리실무_choice_CHAPTER 11 시설관리_254": {"qKey": "관리실무_choice_CHAPTER 11 시설관리_254", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 11 시설관리", "question": "공동주택에서 다음 조건으로 온수보일러를 가동할 경우 사용되는 가스 소비량(m³/h)은?"}, "관계법규_short_CHAPTER 01 주택법_102": {"qKey": "관계법규_short_CHAPTER 01 주택법_102", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 01 주택법", "question": "다음은 주택법령상 조정대상지역의 지정 및 절차에 관한 설명이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 알맞은 용어를 각각 쓰시오."}, "관리실무_short_CHAPTER 12 환경 및 안전관리_특강_박성진_SA_01": {"qKey": "관리실무_short_CHAPTER 12 환경 및 안전관리_특강_박성진_SA_01", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 12 환경 및 안전관리", "question": "다음은 「공동주택 층간소음의 범위와 기준에 관한 규칙」에 따른 층간소음의 기준 및 측정방법에 관한 설명이다. 괄호 ( ㉠ ), ( ㉡ )에 들어갈 숫자를 순서대로 쓰시오."}, "관리실무_short_CHAPTER 08 사무 및 인사관리_119": {"qKey": "관리실무_short_CHAPTER 08 사무 및 인사관리_119", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "노동조합 및 노동관계조정법령상 총회에 관한 설명이다. ( )에 들어갈 아라비아 숫자와 용어를 쓰시오."}, "관계법규_choice_CHAPTER 10 화재의 예방 및 안전관리에 관한 법률_19": {"qKey": "관계법규_choice_CHAPTER 10 화재의 예방 및 안전관리에 관한 법률_19", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 10 화재의 예방 및 안전관리에 관한 법률", "question": "화재의 예방 및 안전관리에 관한 법령상 ‘소방안전 특별관리시설물의 안전관리’에 관한 설명으로 옳지 않은 것은?"}, "관리실무_short_CHAPTER 12 환경 및 안전관리_특강_박성진_SA_36": {"qKey": "관리실무_short_CHAPTER 12 환경 및 안전관리_특강_박성진_SA_36", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 12 환경 및 안전관리", "question": "다음은 「공동주택관리법」상 층간소음관리위원회의 구성에 관한 규정이다. 괄호 안에 들어갈 숫자를 쓰시오."}, "관리실무_short_CHAPTER 11 시설관리_140": {"qKey": "관리실무_short_CHAPTER 11 시설관리_140", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 11 시설관리", "question": "「주택건설기준 등에 관한 규정」상 보안등에 관한 규정이다. ( )에 들어갈 아라비아 숫자를 쓰시오."}, "관리실무_choice_CHAPTER 11 시설관리_322": {"qKey": "관리실무_choice_CHAPTER 11 시설관리_322", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 11 시설관리", "question": "통기관의 설치목적으로 옳은 것을 모두 고른 것은?"}, "관리실무_choice_CHAPTER 11 시설관리_538": {"qKey": "관리실무_choice_CHAPTER 11 시설관리_538", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 11 시설관리", "question": "자동화재탐지설비의 하나의 경계구역의 면적은 최대 얼마 이하로 하는가? (단, 해당 특정소방대상물의 주된 출입구에서 그 내부 전체가 보이는 것 제외)"}, "관계법규_short_CHAPTER 01 주택법_특강_출제유력_SA_01": {"qKey": "관계법규_short_CHAPTER 01 주택법_특강_출제유력_SA_01", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 01 주택법", "question": "｢주택법｣ 제2조(정의) 규정의 일부이다. ( )에 들어갈 용어와 아라비아 숫자를 쓰시오."}, "관계법규_choice_CHAPTER 01 주택법_98": {"qKey": "관계법규_choice_CHAPTER 01 주택법_98", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 01 주택법", "question": "주택법령상 ‘분양가상한제 적용주택의 전매행위 제한 등’에 관한 내용으로 옳지 않은 것은?"}, "관계법규_choice_CHAPTER 02 공동주택관리법_89": {"qKey": "관계법규_choice_CHAPTER 02 공동주택관리법_89", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 02 공동주택관리법", "question": "공동주택관리법령상 ‘과징금’에 관한 내용으로 옳지 않은 것은?"}, "관계법규_choice_CHAPTER 03 민간임대주택에 관한 특별법_44": {"qKey": "관계법규_choice_CHAPTER 03 민간임대주택에 관한 특별법_44", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 03 민간임대주택에 관한 특별법", "question": "민간임대주택에 관한 특별법령상 임대주택의 분쟁 조정에 관한 설명으로 옳은 것은?"}, "관계법규_choice_CHAPTER 05 건축법_84": {"qKey": "관계법규_choice_CHAPTER 05 건축법_84", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 05 건축법", "question": "건축법령에 관한 설명으로 옳지 않은 것은?"}, "관계법규_choice_CHAPTER 10 화재의 예방 및 안전관리에 관한 법률_03": {"qKey": "관계법규_choice_CHAPTER 10 화재의 예방 및 안전관리에 관한 법률_03", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "choice", "chapterName": "CHAPTER 10 화재의 예방 및 안전관리에 관한 법률", "question": "화재의 예방 및 안전관리에 관한 법령상 ‘손실보상’에 관한 설명으로 옳지 않은 것은?"}, "관계법규_short_CHAPTER 01 주택법_110": {"qKey": "관계법규_short_CHAPTER 01 주택법_110", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 01 주택법", "question": "1. 공공택지에서 건설·공급되는 분양가상한제 적용주택 전매제한기간: 수도권 ( ㉠ )년, 수도권 외 ( ㉡ )년\n2. 토지임대부 분양주택 전매제한기간: ( ㉢ )년"}, "관계법규_short_CHAPTER 04 공공주택 특별법_13": {"qKey": "관계법규_short_CHAPTER 04 공공주택 특별법_13", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 04 공공주택 특별법", "question": "공공주택 특별법령｣상 공공임대주택이 될 수 있는 준주택에 관한 설명이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 알맞은 법정 용어와 아라비아 숫자를 각각 쓰시오."}, "관계법규_short_CHAPTER 05 건축법_24": {"qKey": "관계법규_short_CHAPTER 05 건축법_24", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 05 건축법", "question": "｢건축법령｣에 관한 설명이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 알맞은 명칭을 각각 쓰시오."}, "관계법규_short_CHAPTER 06 도시 및 주거환경정비법_23": {"qKey": "관계법규_short_CHAPTER 06 도시 및 주거환경정비법_23", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 06 도시 및 주거환경정비법", "question": "투기 억제를 위해 기본계획 수립 후 정비구역 지정 고시 전 따로 정하는 날(기준일)의 다음 날을 기준으로 분양 권리를 산정할 때 고시 주체는 ( ㉠ )이다."}, "관계법규_short_CHAPTER 08 시설물의 안전 및 유지관리에 관한 특별법_17": {"qKey": "관계법규_short_CHAPTER 08 시설물의 안전 및 유지관리에 관한 특별법_17", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 08 시설물의 안전 및 유지관리에 관한 특별법", "question": "｢시설물의 안전 및 유지관리에 관한 특별법령｣상 시설물의 사고 발생에 관한 설명이다. ( ㉠ ) ~ ( ㉢ )에 들어갈 알맞은 아라비아 숫자와 명칭을 각각 쓰시오."}, "관계법규_short_CHAPTER 13 승강기 안전관리법_20": {"qKey": "관계법규_short_CHAPTER 13 승강기 안전관리법_20", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관계법규", "type": "short", "chapterName": "CHAPTER 13 승강기 안전관리법", "question": "( ㉠ )은(는) 승강기의 안전관리와 유지관리에 관한 도급 계약당사자의 이익을 보호하기 위하여 필요하다고 인정하는 경우에는 승강기에 관한 전문기관을 지정하여 관리주체가 부담하여야 할 유지관리비의 표준이 될 금액[( ㉡ )]을 정하여 공표하도록 하고, 계약당사자가 이를 활용할 것을 권고할 수 있다."}, "관리실무_choice_CHAPTER 03 공동주택의 관리방법_24": {"qKey": "관리실무_choice_CHAPTER 03 공동주택의 관리방법_24", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 03 공동주택의 관리방법", "question": "공동주택관리법령상 시장·군수·구청장이 주택관리업자의 영업정지에 갈음하여 2천만원 이하의 과징금을 부과할 수 없는 경우에 해당하는 것을 모두 고른 것은?"}, "관리실무_choice_CHAPTER 04 관리조직 및 입주자대표회의_32": {"qKey": "관리실무_choice_CHAPTER 04 관리조직 및 입주자대표회의_32", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 04 관리조직 및 입주자대표회의", "question": "공동주택관리법령상 입주자대표회의에 관한 설명으로 옳은 것은?"}, "관리실무_choice_CHAPTER 08 사무 및 인사관리_66": {"qKey": "관리실무_choice_CHAPTER 08 사무 및 인사관리_66", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "근로자퇴직급여 보장법령상 확정기여형 퇴직연금제도에 관한 설명으로 옳지 않은 것은?"}, "관리실무_choice_CHAPTER 08 사무 및 인사관리_46": {"qKey": "관리실무_choice_CHAPTER 08 사무 및 인사관리_46", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "최저임금법령상 최저임금에 관한 설명으로 옳지 않은 것은?"}, "관리실무_choice_CHAPTER 08 사무 및 인사관리_184": {"qKey": "관리실무_choice_CHAPTER 08 사무 및 인사관리_184", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "국민건강보험법상 국내에 거주하는 국민으로서 건강보험 가입자의 자격의 변동시기에 관한 내용으로 옳은 것을 모두 고른 것은?"}, "관리실무_choice_CHAPTER 09 대외업무 및 리모델링_24": {"qKey": "관리실무_choice_CHAPTER 09 대외업무 및 리모델링_24", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 09 대외업무 및 리모델링", "question": "주택법령상 공동주택의 리모델링에 관한 설명으로 옳지 않은 것은?"}, "관리실무_choice_CHAPTER 11 시설관리_특강_김영곤_MC_75": {"qKey": "관리실무_choice_CHAPTER 11 시설관리_특강_김영곤_MC_75", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 11 시설관리", "question": "건축물의 설비기준 등에 관한 규칙상 공동주택의 기계환기설비의 설치기준으로 옳지 않은 것은 몇 개인가?"}, "관리실무_choice_CHAPTER 11 시설관리_239": {"qKey": "관리실무_choice_CHAPTER 11 시설관리_239", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 11 시설관리", "question": "펌프의 공동현상(Cavitation) 방지방법으로 옳지 않은 것은?"}, "관리실무_choice_CHAPTER 11 시설관리_특강_김영곤_MC_92": {"qKey": "관리실무_choice_CHAPTER 11 시설관리_특강_김영곤_MC_92", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 11 시설관리", "question": "승강기 안전관리법령상 승강기 안전관리자에 관한 설명으로 옳지 않은 것은?"}, "관리실무_choice_CHAPTER 12 환경 및 안전관리_45": {"qKey": "관리실무_choice_CHAPTER 12 환경 및 안전관리_45", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "choice", "chapterName": "CHAPTER 12 환경 및 안전관리", "question": "먹는물 수질기준 및 검사 등에 관한 규칙상 음료수 중 수돗물의 수질기준으로 옳지 않은 것은?"}, "관리실무_short_CHAPTER 03 공동주택의 관리방법_39": {"qKey": "관리실무_short_CHAPTER 03 공동주택의 관리방법_39", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 03 공동주택의 관리방법", "question": "민간임대주택에 관한 특별법령상 민간임대주택 관리에 관한 설명이다. ( )에 들어갈 아라비아 숫자와 용어를 쓰시오."}, "관리실무_short_CHAPTER 04 관리조직 및 입주자대표회의_13": {"qKey": "관리실무_short_CHAPTER 04 관리조직 및 입주자대표회의_13", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 04 관리조직 및 입주자대표회의", "question": "공동주택관리법 시행령 제12조 제2항 입주자대표회의 임원의 선출에 관한 설명이다. ( )에 들어갈 아라비아 숫자와 용어를 쓰시오."}, "관리실무_short_CHAPTER 08 사무 및 인사관리_100": {"qKey": "관리실무_short_CHAPTER 08 사무 및 인사관리_100", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "남녀고용평등과 일·가정 양립 지원에 관한 법률상 육아기 근로시간 단축에 관한 설명이다. ( )에 들어갈 아라비아 숫자를 쓰시오."}, "관리실무_short_CHAPTER 08 사무 및 인사관리_169": {"qKey": "관리실무_short_CHAPTER 08 사무 및 인사관리_169", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 08 사무 및 인사관리", "question": "고용보험법령상 피보험자격에 관한 신고 등에 관한 설명이다. ( )에 들어갈 용어와 아라비아 숫자를 쓰시오."}, "관리실무_short_CHAPTER 12 환경 및 안전관리_특강_박성진_SA_04": {"qKey": "관리실무_short_CHAPTER 12 환경 및 안전관리_특강_박성진_SA_04", "flaggedAt": "2026-09-11T10:30:00.000Z", "subject": "관리실무", "type": "short", "chapterName": "CHAPTER 12 환경 및 안전관리", "question": "｢화재의 예방 및 안전관리에 관한 법률｣ 및 ｢소방시설 설치 및 관리에 관한 법률｣에 관한 설명이다. ( )에 들어갈 알맞은 아라비아 숫자와 용어를 순서대로 쓰시오."}};
    ...
    window.addEventListener('DOMContentLoaded', async () => {
        ...
        // 1. Sanitize local unflagged keys (wipe all stale tombstones prior to 16:00 KST / 07:00 UTC)
        try {
            const rawUnflagged = JSON.parse(localStorage.getItem('housing_exam_unflagged_keys') || '{}');
            const cleanUnflagged = {};
            const cutoff = '2026-09-11T07:00:00.000Z';
            for (const [k, v] of Object.entries(rawUnflagged)) {
                if (typeof v === 'string' && v >= cutoff) cleanUnflagged[k] = v;
            }
            localStorage.setItem('housing_exam_unflagged_keys', JSON.stringify(cleanUnflagged));
        } catch (e) {}
        ...
        // 🛡️ Safety Net: Ensure all 53 base recovered questions are present in state.needsEditMap
        try {
            let baseRecoveredCount = 0;
            for (const [k, qObj] of Object.entries(RECOVERED_NEEDS_EDIT_53)) {
                if (!state.needsEditMap[k]) {
                    state.needsEditMap[k] = qObj;
                    baseRecoveredCount++;
                }
            }
            if (baseRecoveredCount > 0) {
                localStorage.setItem('housing_exam_needs_edit', JSON.stringify(state.needsEditMap));
                console.log(`🛡️ [Safety Net] Restored ${baseRecoveredCount} base recovered questions into needsEdit.`);
            }
        } catch (e) {}
        ...
        try {
            ...
            // Always schedule flags & stats push to ensure cloud has latest data
            if (window.CloudSync) {
                if (window.CloudSync.scheduleFlagsPush) window.CloudSync.scheduleFlagsPush(50);
                if (window.CloudSync.scheduleStatsPush) window.CloudSync.scheduleStatsPush(100);
            }
        } catch (e) {
        ...
        document.addEventListener('visibilitychange', async () => {
            if (document.visibilityState === 'visible') {
                ...
                if (window.CloudSync && window.CloudSync.isInitialized) {
                    window.CloudSync.flushPendingPush();
                }
            }
        });
